...
const ConnectionRequests = ({ onOpenChat, socket }) => {
  ...
  if (loading) return <div className="text-slate-500 font-medium p-6 text-center">Loading requests...</div>;

  return (
    <div className="space-y-8">
      <section>
        <h3 className="text-lg font-bold text-slate-900 mb-4">Incoming Requests ({incoming.length})</h3>
        {incoming.length === 0 ? (
          ...
          <div className="space-y-4">
            {incoming.map((req) => (
              <motion.div key={req._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                className="flex items-center justify-between p-4 bg-white/80 backdrop-blur-xl border border-white shadow-md hover:shadow-lg rounded-2xl transition-all">
      ...
      <section>
        <h3 className="text-lg font-bold text-slate-900 mb-4">Outgoing Requests ({outgoing.length})</h3>
        {outgoing.length === 0 ? (
          ...
          <div className="space-y-4">
            {outgoing.map((req) => (
              <div key={req._id} className="flex items-center justify-between p-4 bg-white/60 backdrop-blur-xl border border-white shadow-sm rounded-2xl opacity-80">
                <div className="flex items-center gap-4">
